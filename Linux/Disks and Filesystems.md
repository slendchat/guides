...
#### Расширение тома 2
В данной часте рассматривается расширение рут партиции, в **установленной** системе изнутри ее. Действие происходит в **виртуальной машине** после увеличения дискового пространства.
```bash
sudo parted /dev/sda --script \
  unit s \
  resizepart 3 100%
```
- Это укажет `sda3` занять всё свободное пространство до конца диска.

```bash
sudo partprobe /dev/sda
```
 - Сообщаем ядру о новых границах
...
```
 - Увеличиваем LV на всё свободное место

```bash
sudo resize2fs /dev/ubuntu-vg/ubuntu-lv
```
 - Растягиваем файловую систему (ext4)
...
|Цель|Рекомендуемая связка|
|---|---|
|Стабильный сервер|ext4 + LVM|
|Домашний NAS|btrfs (можно без LVM)|
|Продакшн с бэкапами|ext4 + LVM (или btrfs без LVM)|
|Разработка/тесты|btrfs (удобно со снапшотами)|
|Мини-сервер, простота|ext4 без LVM|
|Большие объёмы данных|xfs + LVM или xfs без LVM|

#### 📁 1. **Мелкие файлы (много маленьких файлов, 4K)**

|ФС|Запись (IOPS)|Чтение (IOPS)|Комментарий|
|---|---|---|---|
|**ext4**|🟢 ~90k|🟢 ~100k|Лучший баланс|
|**xfs**|🟡 ~70k|🟢 ~95k|Хорошо читает, но записи хуже|
|**btrfs**|🔴 ~30-40k|🟡 ~50-60k|Медленнее, особенно при sync|

---

#### 📦 2. **Большие файлы (1GB+)**

|ФС|Seq Write (MB/s)|Seq Read (MB/s)|Комментарий|
|---|---|---|---|
|**xfs**|🟢 ~900|🟢 ~950|Самая быстрая при больших объёмах|
|**ext4**|🟡 ~800|🟡 ~850|Хорошо, но чуть медленнее|
|**btrfs**|🟡 ~750|🟡 ~800|Неплохо, особенно с async|

---

#### 📂 3. **Создание/удаление файлов (например, unpack tar, make)**

|ФС|Время (меньше = лучше)|Комментарий|
|---|---|---|
|**ext4**|🟢 Самое быстрое|Идеально для системных задач|
|**xfs**|🟡 Чуть медленнее|Зависит от настройки inodes|
|**btrfs**|🔴 Значительно дольше|Метаданные + COW = медленное удаление|
